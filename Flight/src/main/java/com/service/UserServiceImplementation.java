...
public class UserServiceImplementation implements UserService{
	...
	public boolean checkLogin(String emailId, String password) {
		...
		if(person!=null)
		{
			...
			String hashed_pass=HashPassword.generateHash(password);
			//if(userPass.equals(HashPassword.generateHash(password)))
			if(userPass.equals(hashed_pass))
				return true;
			else
				return false;
		}
	...
	public boolean checkIfUserExists(String emailId) {
		Person person = personDAO.getUser(emailId);
		if (person != null)
			return false; //i.e user exists
		else
			return true; //i.e user doesn't exist
	}
...
/*
	...
	public boolean registerPerson(Person person) {
		// TODO Auto-generated method stub
		person.setPassword(HashPassword.generateHash(person.getPassword()));
	...
	public List<Booking> getHistory(int personId)
	{		
		List<Booking> bookings=bookingDAO.getBookedDetails(personId);
		return bookings;
	}
